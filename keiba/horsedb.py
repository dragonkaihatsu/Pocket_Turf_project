...
FIELDS = ["馬ID", "馬名", "日付", "場", "R", "レース名", "頭数", "枠番", "馬番",
          ...
          "タイム", "着差", "通過", "ペース", "上り",
          # 不利痕跡の代理B「前走の脚質×4角位置」用（2026-09-21追加）。
          # netkeibaの馬ページ（db.netkeiba.com）にはこの列が無いため、
          # `keiba.cli horses` 経由の全キャリア取得では空のまま。
          # `scripts/build_horse_records.py` が自前収集の出走馬CSVから埋める
          "脚質"]
