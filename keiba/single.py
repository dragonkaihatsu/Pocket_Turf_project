...
実測（大井9-12R 244レース／中央9-12R 1,850レース）で100%を超えるのは
大井の2倍台（ワイド1位-3位 108%）と中央の3倍以上（ワイド1位-3位 126%）だけ。

**的中率は上げられない**という制約がある。1点なので上限は構造的に低く、
中央の全体でいちばん当たる1点でも14%（ワイド1位-2位）、推奨している
3倍以上のワイド1位-3位は10%しか当たらない。正解率を重視するなら
1点買いは向かず、点数を増やすしかない（3頭BOX3点で32%、4頭BOX6点で47%、
6頭BOX15点で71%。data/profiles/*/box_stats.json）。1点買いは
「正解率を捨てて、1レースの損失を賭け金で頭打ちにする」買い方である。
"""
...
def best_single(
    ...
    ok = (best_val["回収率"] >= MIN_RETURN
          ...
          and hits >= MIN_HITS)
    if ok:
        # 正解率を主指標に据えたため、推奨時にも「どれだけ外れるか」を必ず出す。
        # 回収率だけで推奨すると、9割外れる1点を「★」として読まれてしまう
        reason = (f"{tier}の実測で回収率が損益分岐を超えている"
                  f"（的中{hits}本）。ただし的中率は"
                  f"{best_val['的中率']:.0%}＝"
                  f"{1 - best_val['的中率']:.0%}のレースで外れる")
    elif hits < MIN_HITS:
